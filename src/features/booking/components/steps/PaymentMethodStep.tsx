...
import { PromptPayPanel } from "@/features/booking/components/steps/PromptPayPanel";
import type { SubmitBookingResult } from "@/features/booking/components/wizard-submit";

type PaymentMethod = "credit_card" | "cash" | "promptpay";
...
function PaymentOptionCard({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`flex h-15 min-w-[104px] flex-1 cursor-pointer items-center justify-center gap-2 rounded bg-white shadow-[4px_4px_16px_rgba(0,0,0,0.08)] transition-[border-color,transform] duration-150 active:scale-95 lg:h-20 ${
    ...
    <div className="flex w-full flex-col gap-6 rounded border border-[#E4E6ED] bg-white px-4 py-6 lg:gap-10 lg:p-10">
      <div className="flex w-full flex-wrap gap-2 lg:gap-4">
        ...
        <PaymentOptionCard
          ...
          icon={<PromptPayIcon className="h-8 w-8" style={{ color: paymentMethod === "promptpay" ? "#E76B39" : "#9AA1B9" }} />}
          label="PromptPay"
