        ...
        <div key={message.id} className="w-full">
          ...
          {message.suggestion?.format === "Room type" && !message.rooms?.length && message.suggestion.rooms.length > 0 && (
            <div className="mt-3 flex max-w-[320px] flex-wrap gap-2">
              {message.suggestion.rooms.map((room) => <button className="cursor-pointer rounded-full border border-[#ABC0B4] bg-white px-3 py-2 text-sm text-[#465C50] transition-[background-color,transform] duration-150 hover:bg-[#F1F5F2] active:scale-95" key={room} type="button" onClick={() => startsBooking(message) ? props.onOpenMainBooking(room) : props.onSelectSuggestedRoom(room)}>{room}{message.suggestion?.button_name ? " · " + message.suggestion.button_name : ""}</button>)}
            </div>
          )}
        </div>
      ))}
  ...
  return <div className="relative z-[1] -mr-4 mt-4 flex snap-x gap-2 overflow-x-auto pr-4 pb-2">
    {message.rooms?.map((room, index) => <article className="h-[317px] w-[255px] min-w-[255px] snap-start overflow-hidden rounded-lg bg-white shadow-[0_5px_18px_rgba(52,61,78,.08)]" key={room.id}>
      ...
      <div className="flex h-10 items-center bg-[#FAEDE8] p-2">
        {bookingMode ? <button className="flex w-full cursor-pointer items-center justify-center px-2 py-1 text-base leading-4 font-semibold text-[#E76B39] transition-transform duration-150 active:scale-95 disabled:cursor-wait disabled:opacity-60 disabled:active:scale-100" type="button" disabled={isBooking} onClick={() => onStartBooking(room)}>{isBooking ? (locale === "th" ? "กำลังตรวจสอบ..." : "Checking...") : bookNowLabel}</button> : <Link className="flex w-full cursor-pointer items-center justify-between px-2 py-1 text-base leading-4 font-semibold text-[#E76B39] transition-transform duration-150 active:scale-95" href={room.detailHref}>{viewDetailsLabel}<span className="text-2xl font-light leading-4" aria-hidden="true">›</span></Link>}
      </div>
    </article>)}
  </div>;
}
